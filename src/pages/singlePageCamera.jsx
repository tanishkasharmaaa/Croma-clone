...
import { camera } from "../database/camera";
import { NavLink } from "react-router-dom";
export default function SinglePageCamera() {
  const { id } = useParams();
  const [data, setData] = useState(null);

  useEffect(() => {
    // Find the appliance with the matching id
    const selectedCamera = camera.find((cam) => cam.id === parseInt(id));

    // Set the data state with the selected appliance
    setData(selectedCamera);
  }, [id]);
